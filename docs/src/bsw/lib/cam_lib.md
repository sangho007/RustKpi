...
- `libcamera_capture::BufferPool`: 고정 길이 버퍼를 미리 할당하고 `BufferRecycler` 구현을 통해 `CameraBuffer`가 드롭될 때 재사용합니다.
- DTO ↔ Mat 변환은 `rte::lib::camera_lib::{mat_from_buffer, ensure_bgr}`에서 제공하며, `DtoCamRaw::as_mat_view`/`as_bgr_mat`이 해당 유틸을 호출합니다.

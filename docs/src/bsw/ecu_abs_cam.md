...
## 동작 요약
- `spawn_blocking` 내부 루프에서 프레임 캡처 → `Arc<Mat>` 포장 → `DtoCamRaw` 구성 → `raw_tx.send`.
- 프레임 간 간격: 약 33ms(대략 30FPS 목표).
- 소스 전환: `cammode`가 true이면 `PiCamera2`, 아니면 `./video/challenge.mp4` 사용.

## 의존성
- 내부: `bsw::lib::cam_lib` (`FrameCapture` Trait, `PiCamera2` 구현)
- 외부: OpenCV `videoio`, `pyo3/numpy`(PiCamera2 사용 시)

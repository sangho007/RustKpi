...
## 주요 구조체
- `AdasLateralCalibration`: 로컬 경로 대비 스무딩 궤적의 횡오차를 PID 제어하기 위한 게인, 각도 범위, 레이트 리밋, 샘플 인덱스, 채널 인덱스를 제공합니다.
- `AdasLongitudinalCalibration`: 초음파·신호등·경로 상태로 속도를 결정하는 제어 루프 주기, 순항/감속 속도, 감속·정지 거리, 곡률 감속 게인, 속도 레이트 리밋 등을 정의합니다.

## 기본값 특징
- 측면 제어는 중립 90도, ±90도 범위를 기준으로 PID 제어를 수행하며 한 루프당 최대 10도까지 각도 변화를 허용합니다. 스무딩 궤적과 로컬 경로 사이 횡오차(기본 8번째 샘플)를 제어 대상 값으로 사용합니다.

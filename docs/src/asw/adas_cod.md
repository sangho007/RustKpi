...
- `runnable_adas_lateral("ADAS-Lateral", RteChannels)`  
  - 입력: `path.local_tx`, `path.smoothed_tx`  
  - 보정: `AdasLateralCalibration` (횡오차 PID 게인, 샘플 인덱스, 서보 중립/범위, `max_servo_delta_deg`, 대상 서보 채널)  
  - 처리: 로컬 경로를 기준으로 스무딩 궤적의 지정 샘플(기본 8번째)에서 횡방향 오차를 계산하고 PID 제어기로 서보 목표각을 생성합니다. 차선 변경 중에는 PID를 리셋하여 급격한 변화를 방지하며, `max_servo_delta_deg` 레이트 리밋을 적용한 뒤 `DtoServoCtrl`을 송신합니다.  
  - 출력: `control.servo_tx`
...
- 로컬 경로 대비 스무딩 궤적의 횡오차를 PID로 추종하며, 차선 변경 시에는 PID를 리셋해 안정적으로 전환합니다.
